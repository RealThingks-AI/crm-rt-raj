...
export function EditMeetingModal({ meeting, open, onOpenChange, onSubmit, isLoading }: EditMeetingModalProps) {
  ...
  useEffect(() => {
    if (meeting && open) {
      ...
      setStartDate(format(new Date(meeting.start_datetime), 'yyyy-MM-dd'));
      setStartTime(format(new Date(meeting.start_datetime), 'HH:mm'));
      setEndTime(format(new Date(meeting.end_datetime), 'HH:mm'));
